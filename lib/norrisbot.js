...
NorrisBot.prototype._getChannelNameFromID = function(channelID) {
    let channels = this.getChannels()._value.channels
...
NorrisBot.prototype._doSomethingWithAReactionEvent = function (message) {
    ...
    let channelName = this._getChannelNameFromID(item.channel);

    if (message.type === 'reaction_added') {
        count++;
        console.log("New reaction on "+channelName+" :"+message.reaction+":");
    ...
    if (count == 5) {
        ...
        let url = "https://wearebase.slack.com/archives/"+channelName+"/"+messageName;
        let good = "I see that [this message]("+url+") got more than a few reactions, should this be in jira?";
        let attachments = {
            "as_user": true,
            "attachments": JSON.stringify([
                {
                    ...
                    "title_link": "https://wearebase.atlassian.net",
                    "text": "Make a ticket plz.",
                    "color": "#7CD197"
                }
            ])
        }

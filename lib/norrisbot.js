...
NorrisBot.prototype._doSomethingWithAReactionEvent = function (message) {
    ...
    let key = item.channel+"-"+item.ts;
    var count = reactions[key] || 0;
    let itemChannel = this._getChannelById(item.channel);

    if (message.type === 'reaction_added') {
        count++;
        console.log("New reaction on "+itemChannel.name+" :"+message.reaction+":");
    ...
    if (count == 5) {
        let mArr = item.ts.split(".");
        let messageName = "p"+mArr[0]+mArr[1];
        let url = "https://wearebase.slack.com/archives/"+itemChannel.name+"/"+messageName;
        let good = "I have detected a high number of reactions on <"+url+"|this message>.";
        let attachments = {
            "as_user": true,
            "attachments": JSON.stringify([
                {
                    "fallback": "",
                    "title": "Create a ticket on JIRA",
                    "title_link": "https://wearebase.atlassian.net/secure/CreateIssueDetails!init.jspa?pid=11801&issuetype=3&description="+url,
                    "text": "Everyone loves a jira ticket.",
                    "color": "#7CD197"
                }
            ])
        }
        this.postMessageToChannel(itemChannel.name, good, attachments);
    } 
}
